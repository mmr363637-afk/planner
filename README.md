...
| مطالعه | تایمر آزاد و پومودورو (قابل تنظیم)، ادامه‌ی صحیح تایمر هنگام جابه‌جایی بین صفحات/بستن اپ (مبتنی بر timestamp)، ارزیابی یادگیری در پایان، ثبت زمان واقعی، **کارت سریعِ صداهای تمرکز** |
...
| صداهای تمرکز | **White Noise با سنتزِ زنده‌ی Web Audio**: 🌧️ باران، ⛈️ رعد و برق، 🏞️ رودخانه — بی‌پایان و **بدون درزِ تکرار**، بدون فایل صوتی، آفلاین و بدون مصرف اینترنت؛ **میکس هم‌زمان هر سه صدا با حجم مستقل**، ترکیب‌های آماده (طوفان/باران ملایم/کنار رودخانه/میکس کامل)، حجم کلی، و پخشِ سراسری که بین صفحه‌ها قطع نمی‌شود |
...
npm install
npm run dev        # توسعه
npm test           # تست‌ها (Planning Engine, SRS, Streak, Progress, Study time, Jalali, Exam timer, White-noise engine, PWA assets, E2E smoke)
...
src/
  ...
  lib/
    ...
    ambient.ts        # موتور White Noise: سنتز نویزِ دوره‌ای + گراف صوتی باران/رعد/رودخانه + زمان‌بندی رعد
    notify.ts         # Notification/صدا/لرزش
  components/         # UI قابل استفاده مجدد (Card, Button, Modal, ProgressBar, Segmented, TaskRow, JalaliDatePicker, ExamCountdownCard, AmbientMixer, ...)
